script {
    ...
    /// BAKED-IN executor address (`@0xA501`). The target is a literal in the bytecode, so the
    /// proposal's execution hash commits to it and the submitter cannot redirect the removal to a
    /// different executor. Only non-privileged routing values are passed as arguments; see
    ...
    fun main(executor: &signer, timelock_addr: address, proposal_hash: vector<u8>) {
        ...
        timelock::remove_executors(&timelock_signer, vector[@0xA501]);
    }
}
