...
module aptos_experimental::confidential_gas_e2e_helpers {
    ...
    /// Pre-registration variant: the sender does not yet have a confidential asset store, so
    /// `sender_ek` is supplied directly (the key Alice is about to register with) and the
    /// "current" balance is the canonical empty actual balance. Used by Rust e2e tests for the
    /// combined `register_and_confidential_transfer` entrypoint.
    public fun pack_confidential_transfer_proof_pre_registration(
        chain_id: u8,
        sender: address,
        recipient: address,
        sender_dk: &Scalar,
        sender_ek_bytes: vector<u8>,
        transfer_amount: u64,
        new_balance_amount: u128,
        token: Object<Metadata>,
        sender_auditor_hint: vector<u8>,
        ...
        let auditor_eks = build_auditor_list_with_chain_prefix(vector::empty<vector<u8>>());
        let sender_ek = twisted_elgamal::new_pubkey_from_bytes(sender_ek_bytes).extract();
