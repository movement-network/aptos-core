...
module aptos_framework::staking_config {
    ...
    struct StakingRewardsConfig has copy, drop, key {
        ...
        // For example, 0.001% (0.00001) can be represented as 10 / 1000000.
        // Reward APR per year in decimal, must be less than 1. 10% is 0.1 .
        year_apr: FixedPoint64,
    ...
    public fun reward_rate(epoch_interval_secs: u128): (u64, u64) acquires StakingRewardsConfig, StakingConfig {
        get_reward_rate(borrow_global<StakingConfig>(@aptos_framework), epoch_interval_secs)
    }
    ...
    public fun get_reward_rate(
        config: &StakingConfig, epoch_interval_secs: u128
    ): (u64, u64) acquires StakingRewardsConfig {
        if (features::periodical_reward_rate_decrease_enabled()) {
            let rewards_config = borrow_global<StakingRewardsConfig>(@aptos_framework);
            if (fixed_point64::is_zero(rewards_config.year_apr)) {
                (0u64, 1u64)
            } else {
                ...
                // // Very strange nominator is always MAX_REWARDS_RATE , epoch_rewards_rate is eliminated !!!!

                //Remove all calculus with the one of the MIP 125.
                //num_epochs_in_a_year = rewards_rate_period_in_secs / epoch_duration_in_secs
                let num_epochs_in_a_year =
                    (rewards_config.rewards_rate_period_in_secs as u128)
                        / epoch_interval_secs;
                // epoch shouold last less than a year.
                if (num_epochs_in_a_year == 0) {
                    num_epochs_in_a_year = 1;
                };

                // nominator = (target_apr * denominator ) / num_epochs_in_a_year
                let numerator =
                    fixed_point64::multiply_u128(
                        config.rewards_rate_denominator as u128,
                        rewards_config.year_apr
                    );
                let nominator = (numerator as u128) / num_epochs_in_a_year;
                ((nominator as u64), config.rewards_rate_denominator)
            }
        } else {
    ...
    public(friend) fun calculate_and_save_latest_epoch_rewards_rate(): FixedPoint64 acquires StakingRewardsConfig {
        ...
        staking_rewards_config.year_apr
    }
    ...
    fun calculate_and_save_latest_rewards_config(): StakingRewardsConfig acquires StakingRewardsConfig {
        ...
        staking_rewards_config.year_apr = new_rate;
        staking_rewards_config.last_rewards_rate_period_start_in_secs =
            staking_rewards_config.last_rewards_rate_period_start_in_secs
                + staking_rewards_config.rewards_rate_period_in_secs;
        return *staking_rewards_config
    ...
    public fun update_rewards_rate(
        aptos_framework: &signer, year_apr_percent: u64, new_rewards_rate_denominator: u64
    ) acquires StakingConfig {
        assert!(
            !features::periodical_reward_rate_decrease_enabled(),
        ...
        // rate (i.e., rewards_rate / rewards_rate_denominator).
        assert!(
            year_apr_percent <= MAX_REWARDS_RATE,
            error::invalid_argument(EINVALID_REWARDS_RATE)
        );

        // We assert that (rewards_rate / rewards_rate_denominator <= 1).
        assert!(
            year_apr_percent <= new_rewards_rate_denominator,
            error::invalid_argument(EINVALID_REWARDS_RATE)
        );

        // Create the reward config with defaut values.
        // The migration process is:
        // - Use the goverance to update the staking config using , for example, the script aptos-move/move-examples/stake_update_rewards_rate.move
        // - diseable GGP feature and activate periodical reward rate decrease feature.
        // Now the rate decrease evey year with the rewards_rate_decrease_rate.
        // To update it manually call  update_rewards_config() function.
        let min_rewards_rate = fixed_point64::create_from_rational(2, 100); // 2% of min reward rate.
        let rewards_rate_decrease_rate = fixed_point64::create_from_rational(25, 100); // 0,25 per year decrease.
        let new_rewards_rate_denominator = 1_000_000_000;
        let rewards_rate_period_in_secs = 31_536_000; // One year in second

        update_rewards_config(
            aptos_framework,
            fixed_point64::create_from_rational((year_apr_percent as u128), 100),
            min_rewards_rate,
            rewards_rate_period_in_secs,
            rewards_rate_decrease_rate
        );
        ...
        let staking_config = borrow_global_mut<StakingConfig>(@aptos_framework);
        staking_config.rewards_rate = year_apr_percent;
    ...
    public fun update_rewards_config(
        ...
        staking_rewards_config.year_apr = year_apr;
    ...
    fun validate_rewards_config(
        ...
        assert!(
            less_or_equal(year_apr, fixed_point64::create_from_u128((1u128))),
            error::invalid_argument(EINVALID_REWARDS_RATE)
        );
        assert!(
            less_or_equal(min_rewards_rate, year_apr),
