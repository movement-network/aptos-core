...
| 1 | Registration rebuilt on new Lean architecture, old `EvalEquiv/Part*.lean` deleted | 🟡 in progress | day-one axiom-stub commit done; rebuild body nearly feature-complete at the PC-step layer — `EvalEquivRebuild.lean` (2189 lines / **110 theorems**, zero `sorry`, zero axioms) lands: (a) `@[irreducible]` symbolic state with full projection-lemma suite; (b) `eval_registration_eq_run` entry-point reduction; (c) PC-lookup lemmas via `.get?`; (d) **all 55 non-native PCs proved** (full coverage of stLoc/moveLoc/copyLoc/immBorrowLoc/mutBorrowLoc/brFalse/ldConst/ldU64/pop/ret/abort); (e) **all 28 native-call PCs proved (happy paths) + 1 error-path variant** via `StepLemmas.Calls.step_call_native_ret1` / `_none` / `_nativeRef_ret1` / `_nativeRef_ret0` — PCs 1 (some+none), 4, 7, 10, 13, 16, 22, 25, 26, 29, 30, 33, 34, 37, 38, 41, 42, 44, 46, 49, 53, 58, 61, 64, 68, 72, 77, 82. Phase 0 StepLemmas library gained `step_call_native_ret0/ret1/none` and `step_call_nativeRef_ret0/ret1`. Remaining: final composition into `registration_eval_equiv_functional_sim` (thread PC lemmas through `run_succ` to match `verifyRegistrationBytecodeResult`). | rebuild file `lake build` in 2.6s; downstream still builds in 1–2s; target ≤3 min |
| 2 | MSL specs for `*_internal` CA functions (4 functions) | 🟡 in progress | structural-scaffold specs landed for `register_internal` / `deposit_to_internal` / `withdraw_to_internal` / `confidential_transfer_internal` / `rotate_encryption_key_internal` / `normalize_internal` (6 functions — superset of plan scope) — store pre/post, abort conditions, frame conditions. Crypto-layer obligations (balance homomorphism, proof acceptance) deferred to Phase 4 + 5. Verification blocked on Phase 0 ristretto255 patches. | — |
...
| 4 | Lean proofs for `verify_withdrawal_proof` / `verify_transfer_proof` / `verify_normalization_proof` / `verify_rotation_proof` | 🟡 in progress | directories created at `lean/.../Withdrawal/` / `Transfer/` / `Normalization/` / `Rotation/` with `EvalEquiv.lean` scaffold stubs (each documents the shared Phase 4 plan and prerequisite list — requires `MovementFormal.MoveModel.Programs.<Op>` bytecode transcription + oracle structure + functional sim before the rebuild template from Registration can be applied). | stubs build in ~0.2s each |
| 5 | MSL specs for FA-integrated entry points (9 functions) | ☐ pending | — | — |
| 6 | End-to-end composition claims per operation | ☐ pending | — | — |
| 7 | Reproducibility and audit package (§10 deliverables) | 🟡 in progress | scaffolded `audit/` directory: `CLAIMS.md`, `TRUST_BOUNDARIES.md`, `toolchain.lock`, `verify-ca.sh`, `README.md`. `verify-ca.sh --op register --stack lean` works end-to-end; other ops / stacks dispatch to "not implemented yet". | `--op register --stack lean` runs in ~4s cold |
| 8 | Axiom closure (Bulletproofs decision, `registration_eval_equiv_singleton_tail` elimination, ongoing axiom review) | ☐ ongoing | — | — |
...
1. **Who owns the upstream ristretto255 spec patches?** Must check whether upstream accepts a PR or whether we carry a branch-local fix indefinitely. Blocks all Move Prover work on CA.
2. **Bulletproofs: MSL `pragma opaque` with external audit citation, or Lean axiom with external audit citation?** Both are acceptable; pick one and document the trust boundary explicitly. Recommend MSL opaque since Bulletproofs is invoked from Move, not from the Lean bytecode layer.
3. **Does `aptos_framework::fungible_asset` have MSL specs upstream?** `fungible_asset.spec.move` does exist (140 lines, numbered high-level requirements) — but the specs need to be sufficiently expressive for CA to compose against. If FA specs are weaker than CA needs, Phase 5 cost grows significantly. Audit required before committing to Phase 5 timeline.
4. **Registration axiom elimination (`registration_eval_equiv_singleton_tail`):** planned via `dropMs` refactor. Is that prerequisite to Phase 6 composition, or can Phase 6 cite the axiom and close the gap later? Recommend the latter to avoid blocking.
5. **MSL expressiveness for sigma relations:** can MSL state "the transcript bytes at this call site equal `fiat_shamir_sigma_challenge(…)`" in a way Z3 discharges? If not, that becomes an `pragma opaque` boundary with Lean covering the math.
6. **Proof maintenance cost per Move-source diff:** both stacks re-verify on every Move-source change. Estimate churn before committing to the full matrix.
