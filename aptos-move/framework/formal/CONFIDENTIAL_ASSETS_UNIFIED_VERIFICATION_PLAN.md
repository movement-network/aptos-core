...
**Quick Status Reference**: See [`VERIFICATION_STATUS_2026_04_23.md`](VERIFICATION_STATUS_2026_04_23.md) for executive summary, quick commands, key findings, and session progress (updated 2026-04-23). For infrastructure axiom analysis, see [`STEPLEMMAS_AXIOM_ANALYSIS.md`](STEPLEMMAS_AXIOM_ANALYSIS.md).
...
| 8 | Axiom closure (Bulletproofs decision, TEMPORARY axiom elimination, ongoing axiom review) | 🟡 in progress (60% complete) | `audit/AXIOM_INVENTORY.md` current — **62 axioms** across 14 files. Categories: TEMPORARY (5: 1 registration + 4 withdrawal helpers), Phase 4 equivalence (4: bytecode correctness axioms), ConcreteHelpers (26: component behaviors), FunctionalSimBridge (5: architectural bridges), Phase 6 composition (1: register_is_formally_verified, by design), group-theory (12: Edwards group laws + 2 primality facts), Ristretto encoding (4: compression/roundtrip), Bulletproofs (5: external-audit acceptance). **Phase 6 update (2026-04-23):** 4 composition axioms converted to theorems (withdraw/transfer/normalize/rotate). Only TEMPORARY + optional ConcreteHelpers expected to shrink. | **57 permanent + 5 temporary (62 total)**; regenerable via `./audit/verify-ca.sh --coverage` |
...
### Phase 8 — Remaining axioms and closure (ongoing, 60% complete)
- **Status (2026-04-23):** 62 axioms total (57 permanent + 5 TEMPORARY). Phase 6 reduced by 4 (composition axioms → theorems).
...
- **Permanent axioms accepted:** Phase 4 equivalence (4), ConcreteHelpers (26), FunctionalSimBridge (5), crypto (21), Phase 6 composition (1). All documented in `audit/AXIOM_INVENTORY.md` with rationale.
- **Ongoing:** Audit `#print axioms` per release, maintain AXIOM_INVENTORY.md, enforce axiom-diff CI guard.
