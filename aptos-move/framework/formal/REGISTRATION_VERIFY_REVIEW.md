...
- **Refinement theorems** (`…Registration.Refinement`):
  ...
  - `eval_success_implies_prop` — **composition proven (no sorry in proof body)**: composes `eval_eq_func` (via `.dropMs`) + `func_success_implies_exec_some` + `execVerifyRegistrationProof_iff`. Accepts any returned `MachineState` `ms` (not just `MachineState.empty`), since the real bytecode leaves references in the container store. Depends on `eval_eq_func` for the L2≡L1.5 step (which itself uses `registration_eval_equiv_functional_sim` and the singleton-tail axiom `registration_eval_equiv_singleton_tail` on the non-early-error path).
  - `eval_abort_implies_not_prop` — **composition proven (no sorry in proof body)**: composes `eval_eq_func` (via `.dropMs`) + `func_abort_implies_exec_none` + `execVerifyRegistrationProof_iff`. The `.aborted` constructor doesn't carry `MachineState`, so `.dropMs` is trivial. Depends on `eval_eq_func` as above.
  - `eval_eq_func` — **largely proven**: lifts `registration_eval_equiv_functional_sim` at fuel `200` to arbitrary `fuel ≥ 200` via `eval_fuel_ge_dropMs`. Uses `by_cases` on whether `eval` at fuel 200 is `.error`. **One residual sorry** for error-fuel-monotonicity: when the oracle returns garbage (both sides are `.error`), proving `eval fuel = .error` from `eval 200 = .error` requires bounded-execution-length formalization. This case is vacuous for all callers (which assume `eval` returns `.returned` or `.aborted`).
  - `registration_eval_equiv_functional_sim` / `eval_eq_func_200` — **proven** in `EvalEquiv.lean`: `(eval … fuel MachineState.empty).dropMs = verifyRegistrationBytecodeResult …` for `fuel ≥ 200`, using **value args** (struct for ek, not `.immRef`). The singleton compressed-point tail still uses the named axiom `registration_eval_equiv_singleton_tail` for arbitrary oracles; the Schnorr success bundle with canonical EK/commitment bytes is proved separately as `registration_eval_equiv_singleton_tail_of_schnorr_hmac_bundle`. Concrete cross-checks remain in `BytecodeDifftestEval.lean` (`native_decide`).

- **Functional simulation** (`…Registration.FunctionalSim`): `verifyRegistrationBytecodeResult` — a readable Lean function on `MoveValue`s that mirrors the bytecode's control flow, returning `MachineState.empty` in all cases. Serves as the L1.5 intermediary: `eval ≡ L1.5` is verified by `native_decide` on concrete oracles (via `.dropMs`); `L1.5 ≡ L1` is algebraic under oracle coherence. Includes `func_trichotomy`, `buildFSMessageMv` (Fiat-Shamir message construction), `buildFSMessageMv_list` and `buildFSMessageMv_list_gen` (message correctness proven, no sorry). **`MovementFormal.Std.ByteArrayAppend`** proves `byteArray_data_append`, `byteArray_toList_eq_data_toList`, `byteArray_toList_append`, and `byteArray_toList_mk_singleton` from `Init` (local `semireducible` on `ByteArray.toList.loop` only in that module). **`FunctionalSim.ByteArray.*`** re-exports the same `toList` facts under the historical names for `simp`; orthogonal to cryptographic claims.
...
1. **`registration_eval_equiv_singleton_tail` (L2 ≡ L1.5 tail, arbitrary oracle).** After the early-error split and the proved Schnorr success bundle (`registration_eval_equiv_singleton_tail_of_schnorr_hmac_bundle`), this is the single named bridge that `(run from PC 2 …).dropMs` matches `verifyRegistrationBytecodeResult` for **any** oracle satisfying the singleton commitment hypothesis. Elimination is engineering on the `MachineState` / `nativeRef` composition chain (see the elaboration-cost note in `EvalEquiv.lean` above the axiom), not a new crypto assumption. Concrete agreement is still exercised by `native_decide` in `BytecodeDifftestEval.lean`.
...
3. ~~**`ByteArray.toList_*` library axioms**~~ **Resolved:** `MovementFormal.Std.ByteArrayAppend` proves `byteArray_toList_eq_data_toList` / `byteArray_toList_append` / `byteArray_toList_mk_singleton` (plus `byteArray_data_append`) with no axioms on that path.
