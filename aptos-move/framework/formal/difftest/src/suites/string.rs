...
/// Exercises `std::string` UTF-8 natives (`internal_check_utf8`, `internal_is_char_boundary`) and
/// public wrappers (`utf8` + `sub_string` / `index_of`). Semantics match Aptos `move-stdlib/src/natives/string.rs`.
const TEST_SOURCE: &str = r#"
    module 0x1::difftest_string {
        ...
        public fun test_string_internal_is_char_boundary(b: vector<u8>, i: u64): bool {
            string::internal_is_char_boundary(&b, i)
        }
...
impl DiffTestSuite for StringSuite {
    ...
    fn generate_test_cases(&self, storage: &mut InMemoryStorage) -> Result<Vec<TestCase>> {
        ...
        let char_boundary_cases: Vec<(&[u8], u64, &str)> = vec![
            (&[], 0, "empty_i0"),
            (&[], 1, "empty_i_gt_len"),
            (&[0x48, 0x69], 0, "hi_i0"),
            (&[0x48, 0x69], 1, "hi_i1"),
            (&[0x48, 0x69], 2, "hi_i2_end"),
            (&[0x48, 0x69], 3, "hi_i_gt_len"),
            (&[0xe2, 0x82, 0xac], 0, "euro_i0"),
            (&[0xe2, 0x82, 0xac], 1, "euro_i1_cont"),
            (&[0xe2, 0x82, 0xac], 2, "euro_i2_cont"),
            (&[0xe2, 0x82, 0xac], 3, "euro_i3_end"),
            (&[0xff], 0, "invalid_ff_i0"),
            (&[0xff], 1, "invalid_ff_i1_end"),
            (&[99, 97, 102, 195, 169], 3, "cafe_i3_boundary"),
        ];
        for (bytes, i, label) in char_boundary_cases {
            let args = vec![make_u8_vec(bytes), make_u64(i)];
            let result = run_test_case(
                storage,
                STD_ADDR,
                MODULE_NAME,
            ...
            cases.push(TestCase {
                function: format!("test_string_internal_is_char_boundary [{label}]"),
                type_args: None,
                args,
                result,
                skip_lean: false,
