...
def shift_left (bv : MvBitVector) (amount : UInt64) : MvBitVector :=
  ...
  if amount ≥ bv.length then
    ⟨bv.length, Array.replicate n false, by simp⟩
  else
    ⟨bv.length,
     Array.ofFn (n := n) fun i =>
       if h : i.val + k < n then bv.bit_field[i.val + k]'(bv.inv ▸ h) else false,
...
@[simp] theorem new_ok_length {len : UInt64} (h0 : len ≠ 0) (hmax : len < MAX_SIZE)
  ...
  simp only [new, h0, ↓reduceIte, ge_iff_le, hmax, not_le] at hnew
...
@[simp] theorem new_ok_all_false {len : UInt64} (h0 : len ≠ 0) (hmax : len < MAX_SIZE)
  ...
  simp only [new, h0, ↓reduceIte, ge_iff_le, hmax, not_le, ↓reduceIte] at hnew
  have : bv = ⟨len, Array.replicate len.toNat false, by simp⟩ := Except.ok.inj hnew
  simp [this, Array.getElem?_replicate, hi]

theorem set_ok_length {bv bv' : MvBitVector} {i : UInt64}
    (hs : set bv i = .ok bv') : bv'.length = bv.length := by
  simp only [set] at hs; split_ifs at hs with h
  · simp at hs
...
theorem unset_ok_length {bv bv' : MvBitVector} {i : UInt64}
  ...
  simp only [unset] at hs; split_ifs at hs with h
  · simp at hs
  · exact congrArg MvBitVector.length (Except.ok.inj hs)
