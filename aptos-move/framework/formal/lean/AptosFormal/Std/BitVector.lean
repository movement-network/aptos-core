...
private theorem idx_lt_size (bv : MvBitVector) {i : UInt64} (h : ¬ i ≥ bv.length) :
  ...
  rw [bv.inv]
  have hlt : i < bv.length := by
    rwa [ge_iff_le, not_le] at h
  rwa [UInt64.lt_iff_toNat_lt] at hlt
...
@[simp] theorem new_ok_length {len : UInt64} (h0 : len ≠ 0) (hmax : len < MAX_SIZE)
  ...
  have hlt : ¬ MAX_SIZE ≤ len := not_le.mpr hmax
  simp only [hlt, ↓reduceIte] at hnew
...
theorem shift_left_zero (bv : MvBitVector) : shift_left bv 0 = bv := by
  simp only [shift_left, ge_iff_le, UInt64.le_zero_iff, UInt64.toNat_zero]
  by_cases h : bv.length = 0
  · simp only [h, le_refl, ↓reduceIte]
    ext1
    · simp [h]
    · have hsz : bv.bit_field.size = 0 := by rw [bv.inv, h]; rfl
      have hempty : bv.bit_field = #[] := Array.size_eq_zero_iff.mp hsz
      simp [h, hempty]
    · simp [h]
  · have hpos : ¬ bv.length ≤ 0 := by simp [UInt64.le_zero_iff, h]
