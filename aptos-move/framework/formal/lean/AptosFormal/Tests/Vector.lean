...
namespace AptosFormal.Tests.Vector
...
/-! ### real contains (via test wrapper at index 25 in realModuleEnv) -/

theorem real_contains_found :
    returnValues (evalReal 25 [u64Vec [10, 20, 30], .u64 20] 200) ==
      some [MoveValue.bool true] := by
  native_decide

theorem real_contains_not_found :
    returnValues (evalReal 25 [u64Vec [10, 20, 30], .u64 99] 200) ==
      some [MoveValue.bool false] := by
  native_decide

theorem real_contains_empty :
    returnValues (evalReal 25 [u64Vec [], .u64 1] 50) ==
      some [MoveValue.bool false] := by
  native_decide

theorem real_contains_first :
    returnValues (evalReal 25 [u64Vec [42, 10, 20], .u64 42] 100) ==
      some [MoveValue.bool true] := by
  native_decide

/-! ### real index_of (via test wrapper at index 26 in realModuleEnv)
...
theorem real_index_of_found :
    returnValues (evalReal 26 [u64Vec [10, 20, 30], .u64 20] 200) ==
      some [MoveValue.u64 1, MoveValue.bool true] := by
  native_decide

theorem real_index_of_not_found :
    returnValues (evalReal 26 [u64Vec [10, 20, 30], .u64 99] 200) ==
      some [MoveValue.u64 0, MoveValue.bool false] := by
  native_decide

theorem real_index_of_first :
    returnValues (evalReal 26 [u64Vec [42, 10, 20], .u64 42] 100) ==
      some [MoveValue.u64 0, MoveValue.bool true] := by
  native_decide

/-! ### real reverse (via test wrapper at index 27 in realModuleEnv) -/

theorem real_reverse_empty :
    returnValues (evalReal 27 [u64Vec []] 100) ==
      some [u64Vec []] := by
  native_decide

theorem real_reverse_singleton :
    returnValues (evalReal 27 [u64Vec [1]] 100) ==
      some [u64Vec [1]] := by
  native_decide

theorem real_reverse_pair :
    returnValues (evalReal 27 [u64Vec [1, 2]] 100) ==
      some [u64Vec [2, 1]] := by
  native_decide

theorem real_reverse_triple :
    returnValues (evalReal 27 [u64Vec [1, 2, 3]] 150) ==
      some [u64Vec [3, 2, 1]] := by
  native_decide

theorem real_reverse_five :
    returnValues (evalReal 27 [u64Vec [10, 20, 30, 40, 50]] 300) ==
      some [u64Vec [50, 40, 30, 20, 10]] := by
  native_decide

end AptosFormal.Tests.Vector
