...
theorem immRef_value_persistent
    ...
    (refId : Nat)
    (val : MoveValue)
    (h_immRef : ms.containerStore.read? refId = some val)
    ...
    (h_run : run (registrationModuleEnv o) [] frame stack ms fuel =
             .ok [] frame' stack' ms') :
    ms'.containerStore.read? refId = some val := by
...
theorem mutRef_can_update
    ...
    (h_mutRef : ms.containerStore.read? refId = some val_before)
    ...
    (val_after : MoveValue)
    (h_after : ms'.containerStore.read? refId = some val_after) :
...
theorem no_deallocation
    ...
    (refId : Nat)
    (h_exists : ∃ val, ms.containerStore.read? refId = some val)
    (frame' : Frame)
    (stack' : List MoveValue)
    (ms' : MachineState)
    (h_step : step (registrationModuleEnv o) [] frame stack ms =
...
structure ContainerStoreInvariant (ms : MachineState) : Prop where
  ...
  h_valid_refs_have_values : ∀ refId,
    refId < containerStoreSize ms →
    ∃ val, ms.containerStore.read? refId = some val
