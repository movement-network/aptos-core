...
theorem thread_pc6_to_pc8
  ...
  have step7 : step (registrationModuleEnv o)
    ...
    -- Would apply StepLemmas.step_call_nativeRef_ret1 with oracle threading
    sorry  -- Step lemma application needs concrete frame unification
  ...
  have step8 : step (registrationModuleEnv o)
              ...
              .ok { code := verifyRegistrationProofCode, pc := 9,
                ...
                [] []
                { MachineState.empty with containers := containers_after_extract } := by
    -- Would apply StepLemmas.step_stLoc
    sorry  -- Step lemma unification
...
theorem registration_run_pc4_to_pc20_singleton_happy_path
  ...
  -- Thread PC 6 → 8
  -- Note: s6.v = s4.v = v by definition, so hv_struct about v also applies to s6.v
  obtain ⟨s8, _⟩ := thread_pc6_to_pc8 s6 rCompressed restData sorry

  -- Thread PC 8 → 11
  obtain ⟨s11, _⟩ := thread_pc8_to_pc11 s8 respBa_val scalar restScalarData horacle_scalar

  -- Thread PC 11 → 15
  have h_s11_opt : ∃ scalar restData, s11.s_opt = MoveValue.struct_ (MoveValue.bool true :: scalar :: restData) := sorry
  obtain ⟨s15, _⟩ := thread_pc11_to_pc15 s11 h_s11_opt

  -- Thread PC 15 → 18
  have h_s15_opt : ∃ restData, s15.s_opt = MoveValue.struct_ (MoveValue.bool true :: s15.scalar :: restData) := sorry
  obtain ⟨s18, _⟩ := thread_pc15_to_pc18 s15 h_s15_opt
