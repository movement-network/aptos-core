...
the `FuncBody.native` calling convention established by `MoveModel/Native.lean`.
...
open MovementFormal.Std.Option       (MoveOption)
...
private def moveOptionToMvOption (_ : MoveType) : MoveValue → Option (MoveOption MoveValue)
