...
/-- Chain four copyLoc operations.

All four locals remain unchanged since copyLoc preserves values.
-/
theorem chain_four_copyLoc
    (frame : Frame) (cs : List Frame) (rest : List MoveValue) (ms : MachineState)
    (n i1 i2 i3 i4 : Nat)
  ...
  have h := run_succ_four_ok fuel frame1 frame2 frame3 { frame3 with pc := n + 4 }
