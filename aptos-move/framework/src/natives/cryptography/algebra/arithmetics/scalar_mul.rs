...
/// Returns the MSM window size, or `None` when `log2_ceil` cannot be computed.
        ...
        let window_size = match ark_msm_window_size(num_entries) {
            ...
            None => {
                return Err(SafeNativeError::Abort {
                    abort_code: E_SCALAR_MUL_MSM_WINDOW_SIZE_FAILED,
                })
            },
        };
        ...
        let new_element: $element_typ =
            ark_ec::VariableBaseMSM::msm(bases.as_slice(), scalars.as_slice()).map_err(|_| {
                SafeNativeError::Abort {
                    abort_code: E_SCALAR_MUL_MSM_COMPUTATION_FAILED,
                }
            })?;
        let new_handle = store_element!($context, new_element)?;
        Ok(smallvec![Value::u64(new_handle as u64)])
