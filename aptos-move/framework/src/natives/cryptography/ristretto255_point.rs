...
/// Internal error: handle does not refer to an allocated point.
const E_INVALID_POINT_HANDLE: u64 = 0x0A_0001;

/// Internal error: log2 computation failed during multi-scalar mul gas costing.
const E_MSM_LOG2_FAILED: u64 = 0x0A_0002;

/// Internal error: same handle passed twice to `get_two_muts`.
const E_RISTRETTO255_POINT_DUPLICATE_HANDLE: u64 = 0x0A_0003;

/// Internal error: invalid Ristretto scalar bytes length.
...
impl PointStore {
    ...
    /// Gets a RistrettoPoint that was previously allocated, or aborts if the handle is invalid.
    pub fn get_point(&self, handle: &RistrettoPointHandle) -> SafeNativeResult<&RistrettoPoint> {
        self.points
            .get(handle.0 as usize)
            .ok_or(SafeNativeError::Abort {
                abort_code: E_INVALID_POINT_HANDLE,
            })
    }

    /// Gets a mutable RistrettoPoint that was previously allocated, or aborts if the handle is invalid.
    ...
    /// Returns an error if the same handle is passed twice or either handle is out of bounds.
    pub fn get_two_muts(
        &mut self,
        ...
        let (sw, a, b) = match Ord::cmp(a, b) {
            ...
            Ordering::Equal => {
                return Err(SafeNativeError::Abort {
                    abort_code: E_RISTRETTO255_POINT_DUPLICATE_HANDLE,
                })
            },
        };

        let split_idx = a + 1;
        if split_idx > self.points.len() {
            return Err(SafeNativeError::Abort {
                abort_code: E_INVALID_POINT_HANDLE,
            });
        }
        let (left, right) = self.points.split_at_mut(split_idx);
        let a_ref = left.get_mut(a).ok_or(SafeNativeError::Abort {
            abort_code: E_INVALID_POINT_HANDLE,
        })?;
        let b_ref = right
            .get_mut(b - split_idx)
...
pub(crate) fn safe_native_multi_scalar_mul_no_floating_point(
    ...
    let log2_val = log2_floor(num + 1).ok_or(SafeNativeError::Abort {
        abort_code: E_MSM_LOG2_FAILED,
    })?;
    context.charge(
