...
fn move_framework_unit_tests() {
    run_tests_for_pkg("aptos-framework", false, BTreeMap::new());
}

#[test]
fn move_aptos_stdlib_unit_tests() {
    run_tests_for_pkg("aptos-stdlib", false, BTreeMap::new());
}

#[test]
fn move_stdlib_unit_tests() {
    run_tests_for_pkg("move-stdlib", false, BTreeMap::new());
}

#[test]
fn move_token_unit_tests() {
    run_tests_for_pkg("aptos-token", false, BTreeMap::new());
}

#[test]
fn move_token_objects_unit_tests() {
    run_tests_for_pkg("aptos-token-objects", false, BTreeMap::new());
}
...
fn move_experimental_unit_tests() {
    run_tests_for_pkg(
        ...
        BTreeMap::from([(
            ...
            AccountAddress::from_hex_literal("0x7").unwrap(),
        )]),
    );
}
